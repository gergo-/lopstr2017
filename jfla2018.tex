...
% Gabriel: (c) pourrait être égal à (true) ou (false) pour tous les
% chemins, auquel cas une des branches est effectivement morte. Il
% s'agit ici d'une approximation.
...
% Gabriel: Pourquoi n'a-t-on pas FV(c) dans livein{S},
% puisqu'exécuter la boucle commence par tester c ?
%
% Note: si on avait livein{S} := livein{B} U liveout{S} U FV(c),
% alors on pourrait avoir liveout{B} := livein{S}
    \liveout{S} \neq \emptyset
}{
    \triple{\livein{S}}
...
% Gabriel: je ne trouve pas clair de voir pourquoi les conditions de la forme
%   S \neq \emptyset
% sont nécessaires.
%
% Celle sur la séquence interdit d'écrire (return v; S), où S serait
% du code mort. C'est utile.
% Mais pourquoi les conditions sur Skip et While ?
%
% Peut-être que tu pourrais détailler le sens ou expliquer la
% nécessité de *toutes* ces conditions de la deuxième sorte.
...
% Gabriel: je pense que rappeler la règle d'inférence pour 'while' ici
% aiderait le lecteur à suivre.
...
fin.
% Gabriel: Il me semble que le générateur utilise
%   liveout{S} U B' U FV(c)
% plutôt que
%   liveout{S} U livein{B} U FV(c)
...
% Gabriel: Implantation ou Implémentation ? (Moi j'utilise plutôt
% implémentation, mais peut-être as-tu une préférence claire pour
% Implantation ?)
